...
var EXTENSIONS_REGEXP = /(?:^\s*(?:\d\.\s+)?|\s+[23]\.\s+)[Ff]ile [Ee]xtension(?:\(s\)|s|)\s?:\s+(?:\*\.|\.|)([0-9a-z_-]+|[0-9A-Z_-]+)(?:\s+\([^)]+\))?(?:(?:\s+(?:and|or)|\s*,|)\s+(?:\*\.|\.|)([0-9a-z_-]+|[0-9A-Z_-]+)|\s*,\s*(?:\*\.|\.|)([0-9a-z_-]+|[0-9A-Z_-]+)(?:\s*,\s*(?:\*\.|\.|)([0-9a-z_-]+|[0-9A-Z_-]+))?(?:\s*,\s*(?:\*\.|\.|)([0-9a-z_-]+|[0-9A-Z_-]+))?)?(?:\s*[34]\.\s+|\s+[A-Z(]|\s+(?:are(?:\s+both)?|is)\s+declared|\s*$)/m
var EXTENSIONS_QUOTED_REGEXP = /^\s*(?:\d\.\s+)?[Ff]ile [Ee]xtension(?:\(s\)|s|)\s?:\s*(?:\w+|(?:[^0-9'"\r\n\s]+\s+)+(?:extensions?|of)\s+)?(?:"\.?([0-9a-z_-]+)"|'\.?([0-9a-z_-]+)')(?:,?\s+(?:(?:and|or)\s+)?(?:"\.?([0-9a-z_-]+)"|'\.?([0-9a-z_-]+)'))?/im
